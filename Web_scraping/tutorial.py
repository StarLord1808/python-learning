...
def get_film_names(query: str, max_results: int = 10):
    films = []
    with DDGS() as ddgs:
        results = ddgs.text(f"{query} film", max_results=max_results)
        print(results)
        for r in results:
...
if __name__ == "__main__":
    movies = get_film_names("Review of Superman(2025)", max_results=5)
